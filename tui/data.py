...
def group_into_sweeps(
    ...
    """Group sequential ResultSets into sweeps.

    Runs whose timestamps are within *gap_hours* of each other are
    considered part of the same sweep.  Input should be one bench_type.
    Returns newest-sweep-first; each sweep is internally newest-first.
    """
    if not result_sets:
        return []

    by_time = sorted(result_sets, key=lambda rs: rs.timestamp)
    gap = _td(hours=gap_hours)

    buckets: list[list[ResultSet]] = [[by_time[0]]]
    for rs in by_time[1:]:
        prev = _parse_ts(buckets[-1][-1].timestamp)
